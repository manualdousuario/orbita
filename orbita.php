...
function orbita_link_options( $url = '', $title = '' ) {
	...
	$host = wp_parse_url( str_replace( 'www.', '', get_bloginfo('url') ), PHP_URL_HOST );
		...
		foreach ( $tags as $tag ){
			if ( str_contains( $title, '[' . $tag . ']' )){
				if(isset($options['paywall'])) {
					...
					$options['translate'] = 'https://translate.google.com/translate?sl=' . $tag . '&tl=pt&hl=pt-BR&u=' . $url;
				}
			}
		}
	}
...
function orbita_convert_image( $image ) {
	...
	if( $image_width > $max_size || $image_height > $max_size ) {
		...
		imagecopyresampled( $new_image, $image_original, 0, 0, 0, 0, $image_width / $ratio, $image_height / $ratio, $image_width, $image_height );
	} else {
